...
| Offline test suite | All 162 tests run without any network calls, using unittest.mock, async fake HTTP clients, and saved fixture files. Real pypdf, python-docx, and Pillow are exercised against in-memory synthetic file blobs, providing genuine parser coverage without live downloads. |
| Scan authorization audit trail | Records whether each scan was interactively authorized (`INTERACTIVE`), bypassed (`BYPASSED` via `--no-confirm`), or synthetic (`MOCK`) directly into SQLite `scan_runs.confirmation_method`. Surfaced in terminal footers and HTML report hero badges to ensure clear forensic accountability for automated and manual scans alike. |
| Dual-tier operational interface | Combines an animated Rich terminal interface (with active module progress bars and high-signal summary tables) with optional self-contained dark-mode HTML reports, supporting both rapid CLI triage and comprehensive audit reporting. |
