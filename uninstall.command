#!/usr/bin/env bash
# =============================================================================
#  uninstall.command — OSINT Recon Suite safe uninstaller (macOS Finder)
# =============================================================================
#
#  Double-click in Finder to open Terminal and run the interactive uninstaller.
#
#  FIRST-TIME SETUP (one-time, only needed once):
#    If macOS blocks this file, right-click → Open → Open in the Finder dialog.
#    Or run in Terminal:  chmod +x uninstall.command
#
#  MENU:
#    Option 1 — Clean scan data (reports, DB, logs, caches) — keeps code & venv
#    Option 2 — Full removal (scan data + venv + entire repository directory)
#
# =============================================================================
...
# Move to the directory containing this script (Finder sets CWD to $HOME)
...
clean_scan_data() {
    ...
    if [[ -f "${SCRIPT_DIR}/osint_recon.log" ]]; then
        rm -f "${SCRIPT_DIR}/osint_recon.log"; deleted "osint_recon.log"
    else
    ...
    if [[ -f "${SCRIPT_DIR}/.env" ]]; then
        ...
        if [[ "${RM_ENV,,}" == "y" ]]; then
            rm -f "${SCRIPT_DIR}/.env"; deleted ".env"
        else
            info ".env kept."
        fi
    fi
    ...
    if [[ -d "${SCRIPT_DIR}/.pytest_cache" ]]; then
        rm -rf "${SCRIPT_DIR}/.pytest_cache"; deleted ".pytest_cache/"
    else
        info ".pytest_cache — nothing to remove."
    fi

    [[ -f "${SCRIPT_DIR}/venv/.install_stamp" ]] && rm -f "${SCRIPT_DIR}/venv/.install_stamp"
...
full_removal() {
    ...
    if [[ -d "${SCRIPT_DIR}/venv" ]]; then
        rm -rf "${SCRIPT_DIR}/venv"; deleted "venv/"
    else
    ...
    echo ""
    info "Reinstall at any time with:"
